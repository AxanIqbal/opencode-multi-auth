...
export class AccountManager {
    ...
    importFromOpenCodeAuth() {
        const auth = readJSON(OPENCODE_AUTH_FILE);
        if (!auth)
            return;
        const oa = auth.openai;
        if (oa?.type === "oauth" && oa.refresh) {
            // Deduplicate by refresh token
            if (this.accounts.some((a) => a.refresh === oa.refresh))
                return;
    ...
    addAccount(refreshToken, accessToken, expires, label) {
        ...
        if (existing) {
            ...
            if (info.planType)
                existing.planType = info.planType;
            if (email)
    ...
    async selectExcluding(exclude, model) {
        if (this.accounts.length === 0)
            return null;
        const now = Date.now();
    ...
    markRateLimited(account, cooldownMs, model) {
    ...
    async _doRefresh(account) {
        try {
            ...
            if (result.type === "success") {
                ...
                if (info.planType)
                    account.planType = info.planType;
                if (info.email)
            ...
            if (code === "refresh_token_reused" || code === "invalid_grant") {
                ...
                if (!this.config.quietMode) {
                    console.error(`[multi-auth] Account ${account.label || account.email || account.index} needs re-auth (${code})`);
                }
            }
            else {
    ...
    getBound(index, model) {
        const acct = this.accounts.find((a) => a.index === index);
        if (!acct)
            return null;
        if (this.isAvailable(acct, model, Date.now()))
            return acct;
        return null;
    }
    // ── Private helpers ──────────────────────────────────────
